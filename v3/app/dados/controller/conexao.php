...
class BancoDeDados {
	...
	public function conectar() {
		//$this->conexao = mysqli_connect($this->host, $this->usuario, $this->senha, $this->bd);
		$this->conexao = mysqli_connect("mysql", "root", "Senha123", "bd_agenda");
	...
	public function resultado($rs, $indice, $campo) {
		$retorno = mysqli_result($rs, $indice, $campo);
		return utf8_encode($retorno);
	}

	public function rows($rs) {
		$retorno = mysqli_num_rows($rs);
		return $retorno;
	}
	...
	public function selecionar($campos, $tabela, $where="", $ordena="", $limite="") {
		...
		for($j=0; $j<$this->rows($rs_colunas); $j++) {
			$colunas = mysqli_fetch_assoc($rs_colunas);
